\
"""
AI Integration Service: Daily India Crime & Accidents Ingest Pipeline (V3 - Stable Chat Workflow)
Uses official `google-genai` SDK with Web Search Grounding to fetch live,
severity-prioritized incidents across all Indian regions via a stable Chat session.
"""
...
BACKEND_API_URL = os.getenv("BACKEND_API_URL", "http://localhost:3000/api/records")
...
Search local terms: விபத்து (Accident), கொலை (Murder), അപകടം (Accident), കൊലപാதകം (Murder).
    """,
    "South_Karnataka": """
SOURCE DIRECTIVE:
...
def fetch_region_incidents(
    ...
    """Fetches incident news via stable Chat session to bypass grounding restrictions."""
    states_str = ", ".join(states_list)
    media_instruction = REGIONAL_MEDIA_INSTRUCTIONS.get(region_name, "")
    print(f"[*] Extracting ground-truth telemetry for region [{region_name}]...")
...
4. STRICT DATE GUARDRAIL: ONLY extract incidents that actually occurred on {yesterday_str} or {target_date}. IGNORE historical accidents even if a news website re-posted or re-indexed the article today.
5. Translate any vernacular news summary into English.
6. Output ONLY a valid raw JSON array with no conversational text or markdown explanation.
...
"""
    ...
    for attempt in range(1, max_retries + 1):
        try:
            # Create standard chat session which allows clean automatic function calling with web grounding
            ...
            if not raw_text:
                print(f"[!] Region {region_name} returned an empty response.", file=sys.stderr)
                return []

            # Clean JSON response markers from output string safely
...
def fetch_all_india_data(target_date: str) -> list:
    ...
    for region_name, states in REGIONAL_GROUPS.items():
        ...
        print(f"[✓] Extracted {len(records)} verified entries from [{region_name}].")
...
def main():
    print("=========================================================================")
    print("  Production AI Ground-Truth Ingest Telemetry Script Pipeline (V3)   ")
    print("=========================================================================")

    today_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")

    # 1. Fetch raw data across all regional groups (using chat architecture)
    ...
    if raw_records:
        ...
        # 3. Push to prefixed backend path target route
